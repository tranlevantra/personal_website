...
author: "Tra Tran"
date: '2024-12-11'
url: "https://tranlevantra.github.io/blogs/#sensitivity-analysis"
...
-   Brier Score: The mean squared difference between predicted probabilities and actual class labels

$$1/n * Σ (P - o)^2$$

-   Logarithmic Loss (Log Loss): The negative log-likelihood of the predicted probabilities given the actual class labels

$$-1/n * Σ [o * log(P) + (1 - o) * log(1 - P)]$$

-   RMS Calibration Error: The root mean squared difference between predicted probabilities and actual class labels
...
-   Specificity (Positive Predictive Value): $\frac{TP} {TP + FP}$
...
For example, in a breast cancer classification test dataset with 90% of instances are those without cancer (class 0) and just 10% are with cancer (class 1), a model that predicts all people to be cancer-free would achieve an accuracy of 90%. However, this model would be useless as it fails to identify any cancer cases.
...
![Figure 4: The reliability diagram shows that positive class accuracy metrics are highly sensitive to threshold changes due to the wider spread of predicted probabilities. In contrast, the negative class has tightly clustered probabilities near zero, making its metrics more stable.](figures/threshold_animation.gif){width="80%" alt-text=""}

## Some Notes

This article is not meant to be an exhaustive guide to model performance assessment in binary classification, but rather a reflection on often overlooked aspects — particularly the role of calibration metrics and sensitivity to decision thresholds.

The idea of plotting predicted probabilities against actual class labels to create a reliability diagram stems from my own experimentation. All example visualisations in this article were created using [ggplot2](https://ggplot2.tidyverse.org).
